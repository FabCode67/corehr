...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(`${apiBaseUrl}/hr-analytics/export/snapshot-pdf${request.nextUrl.search}`, { cache: "no-store" })
  ...
  return new NextResponse(buffer, {
    headers: {
      ...
      "Content-Disposition": response.headers.get("content-disposition") ?? `attachment; filename="hr-statistics-snapshot.pdf"`,
    },
  })
}
